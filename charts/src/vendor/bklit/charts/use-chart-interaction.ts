...
export function useChartInteraction({
  ...
  // Keep a single-finger tap visible. Clearing on every touchend made a tap
  // appear to do nothing on phones; two-finger selection still clears it.
  const singleTouchRef = useRef(false);
  ...
  const handleTouchEnd = useCallback(() => {
    if (singleTouchRef.current) {
      singleTouchRef.current = false;
      // Keep a one-tap sample on screen until the next interaction. This is
      // the touch equivalent of the desktop click handler.
      return;
    }
    clearTooltip();
    setSelection(null);
  ...
  const interactionStyle: React.CSSProperties = {
    ...
    // Let a vertical swipe continue scrolling the status page. Taps still
    // select the nearest sample through the click/touch-start resolver.
    touchAction: "pan-y",
  };
