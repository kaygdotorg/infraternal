...
const ChartCore = memo(function ChartCore({
  ...
  // Mouse/touch move handler. The registry's bar interaction was mouse-only;
  // keeping the same nearest-band resolver for touch and click makes the
  // mobile chart usable without adding a second tooltip implementation.
  const handleMouseMove = useCallback(
    (
      ...
      const point =
        "touches" in event
          ? event.touches[0] && event.currentTarget.ownerSVGElement
            ? localPoint(
