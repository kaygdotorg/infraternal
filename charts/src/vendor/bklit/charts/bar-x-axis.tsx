...
export interface BarXAxisProps {
  ...
  maxLabels?: number;
  /** Optional formatter for categorical labels (for example epoch timestamps). */
...
const BarXAxisInner = memo(function BarXAxisInner({
  ...
  const labelsToShow = useMemo(() => {
    ...
    const allLabels = data.map((d, index) => {
      ...
      return { label: formatLabel?.(label, index) ?? label, x };
    });
    ...
    // Otherwise, choose evenly spaced labels while retaining both domain
    // boundaries so a sparse detail chart still shows its final/Now sample.
    const count = Math.max(2, maxLabels);
    const indices = Array.from({ length: count }, (_, i) =>
      Math.round((i / (count - 1)) * (allLabels.length - 1))
    );
