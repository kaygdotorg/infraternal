...
export interface XAxisProps {
  ...
  /** Optional timestamp formatter for axis and date-ticker labels. */
...
function appendProjectionTailTicks(
  ...
  const endLabel = formatDate(domainEnd);
...
const XAxisInner = memo(function XAxisInner({
  ...
  const { xScale, margin, tooltipData, data, xAccessor, dateLabels, xDomain } =
    useChart();
  const formatDate = formatLabel ?? ((date: Date) => shortDateFmt.format(date));
  ...
  const hoveredLabel =
    isHovering && tooltipData
      ? formatDate(xAccessor(tooltipData.point))
      : null;
